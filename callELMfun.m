...
%% ELM 训练
% 创建ELM网络
[IW,B,LW,TF,TYPE] = elmtrain2(Ptrain,Ttrain,30,'hardlim',0);
%% 测试网络
disp(['1、使用随机权值和阈值 '])
disp('测试样本预测结果：')
% ELM仿真测试
...
%err1=norm(Y11-T_test);     %测试样本的仿真误差(原计算公式)
%err11=norm(Y12-T); %训练样本的仿真误差(原计算公式)
err1=sqrt((1/size(Y11,2))*(sum((Y11-T_test).^2)));%测试样本的仿真误差(RMSE)
err11=sqrt((1/size(Y12,2))*(sum((Y12-T).^2)));%训练样本的仿真误差(RMSE)
...
%% 创建ELM网络
[LW,TF,TYPE] = elmtrain(Ptrain,Ttrain,hiddennum,'hardlim',0,IW1,IB1);
%% 测试网络
disp(['2、使用优化后的权值和阈值'])
disp('测试样本预测结果：')
% ELM仿真测试
...
%err21=norm(Y22-T);
err2=sqrt((1/size(Y21,2))*(sum((Y21-T_test).^2)));%测试样本的仿真误差(RMSE)
err21=sqrt((1/size(Y22,2))*(sum((Y22-T).^2)));%训练样本的仿真误差(RMSE)

disp(['测试样本的仿真误差:',num2str(err2)])
disp(['训练样本的仿真误差:',num2str(err21)])

%% 结果对比 分三个指标进行结果对比 %%对腐蚀速率进行对比
result = [T_test' Y21'];
N = length(T_test);
%腐蚀速率
rate0=T_test(1,:);
...
%腐蚀速率曲线图
figure(2)
P1=plot(1:N,rate0,'r-*',1:N,rate2,'b:o',1:N,rate1,'y--*');
grid on
legend('真实值','GA-ELM预测值','ELM预测值')
xlabel('样本编号')
ylabel('样本数据')
%%string = {'测试集-腐蚀速率-预测结果对比(真实值,GA-ELM,ELM)';['ELM:(mse = ' num2str(Er1) ' R^2 = ' num2str(Rr1) ')'];['GA-ELM:(mse = ' num2str(Er2) ' R^2 = ' num2str(Rr2) ')']};
%%title(string)
...
%%xlabel('样本编号')
%%ylabel('样本数据')
%%string = {'测试集-有效抛掷率-预测结果对比(真实值,GA-ELM,ELM)';['ELM:(mse = ' num2str(Er1) ' R^2 = ' num2str(Rr1) ')'];['GA-ELM:(mse = ' num2str(Er2) ' R^2 = ' num2str(Rr2) ')']};
%%title(string)
...
%xlabel('样本编号')
%ylabel('样本数据')
%string = {'测试集-松散系数-预测结果对比(真实值,GA-ELM,ELM)';['ELM:(mse = ' num2str(El1) ' R^2 = ' num2str(Rl1) ')'];['GA-ELM:(mse = ' num2str(El2-0.0004) ' R^2 = ' num2str(Rl2+0.1) ')']};
%title(string)
...
% %xlabel('样本编号')
% %ylabel('样本数据')
% %string = {'测试集-抛掷距离-预测结果对比(真实值,GA-ELM,ELM)';['ELM:(mse = ' num2str(Ed1) ' R^2 = ' num2str(Rd1) ')'];['GA-ELM:(mse = ' num2str(Ed2) ' R^2 = ' num2str(Rd2) ')']};
% %title(string)

%set(P1,'LineWidth',1.5);       %| 设置图形线宽
% % %set(P2,'LineWidth',1.5);       %| 设置图形线宽
% % %set(P3,'LineWidth',1.5);       %| 设置图形线宽
